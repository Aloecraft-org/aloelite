...
# -- 1. the manager -------------------------------------------------------
# --host 127.0.0.1 keeps WebDAV legal without TLS: its Basic auth carries the
# volume PIN, so aloelite REFUSES to serve it off loopback in the clear. If you
# later want S3 reachable on the VPN, run a SECOND manager with its own --root
# and no --webdav; see doc/WINDOWS.md.
...
`$ErrorActionPreference = 'Stop'
$($env_lines -join "`n")
& '$Python' -m manager.web --webdav --host 127.0.0.1 --port $Port --root '$Root'
