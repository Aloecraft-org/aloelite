...
# `-m aloelite.cli web`, NOT `-m manager.web`. Both spell the same thing on a
# current build, but manager/web.py only gained its `__main__` guard after
# 0.3.6 -- on a release without it, `python -m manager.web` imports the module,
# runs nothing, and exits 0. A task that silently succeeds while starting no
# server is the worst available failure, so this uses the entry point that has
# worked since 0.3.0.
...
`$ErrorActionPreference = 'Stop'
$($env_lines -join "`n")
& '$Python' -m aloelite.cli web --webdav --host $BindHost --port $Port --root '$Root'$insecureFlag
...
# that actually works.
# Where THIS machine reaches its own manager. Binding 0.0.0.0 listens on
# every interface, so loopback is right and stays right if the VPN address
# changes. Binding a SPECIFIC address means the server is not on loopback at
# all, so the drive and the health probe must use that address too -- getting
# this wrong is a mapping that fails with a connection refused nobody expects.
$localHost = if ($BindHost -eq '0.0.0.0') { '127.0.0.1' } else { $BindHost }
...
net use ${Drive}: \\$localHost@$Port\DavWWWRoot\dav\$VolumeId /persistent:no
