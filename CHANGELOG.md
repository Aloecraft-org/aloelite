...
**`directory_listing` was O(N^2) and is now linear.** NODE-5 says the
greatest `node_id` per effective name is the visible one, and the view
derived that with a correlated scalar subquery: for every row, re-read
the container's whole child set and take the max. As a window function
it is one pass --
`max(n.node_id) OVER (PARTITION BY ae.from_id, coalesce(ae.name, n.name))`.

One `readdir`, measured against ext4 on the same disk:

| entries | ext4 | before | after |
|---|---|---|---|
| 1,000 | 0.33 ms | 375 ms | 8.2 ms |
| 5,000 | 1.63 ms | 10.2 s | 41 ms |
| 10,000 | 2.81 ms | 51.0 s | 96 ms |

532x at 10,000 entries through the library, 103x through the Python
mount and 342x through the Rust one, and the gap widens with N because
the shape changed rather than the constant. Per-entry cost is now flat
(8.2 -> 9.6 us), which is what linear looks like. Writes are untouched:
a view is not an index, and `create_ops` and the ingest curve are
unchanged.
...
(D-5) and hidden same-name siblings, which are the two cases `visible`
exists for.

The view is DROPped and recreated rather than `CREATE VIEW IF NOT
EXISTS`, inside a transaction, so an existing file picks the rewrite up
on its next open without a schema-era bump and without a concurrent
opener ever observing the gap between the two statements. A view holds
no data and this one returns exactly what the old one did, so an older
build loses nothing: it reinstalls its own equivalent definition.

**The obvious fix does not work, and was measured before being
discarded.** An index on `node (name)` is ignored by the planner
outright; with `ANALYZE` it is used and picks a WORSE shape -- a full
`SCAN edge` for the outer query -- making `stat` slower (1.53 -> 2.57 ms)
while barely moving `readdir`. No index on one table can serve a
predicate that spans two.

**Lookup is still linear and is NOT fixed by this.** `resolve_segment`
and `resolve_path` carry the same two-table predicate and do not go
through the view: finding a child by name still scans the container.
6.3 ms at 10,000 entries against ext4's 0.005 ms, 1,170x. Fixing it
needs `edge.name` populated unconditionally so `edge (from_id, name)
WHERE archived = 0` becomes a real covering index -- a data migration
and an era bump, which is era-3 work.

### FUSE: cache the directory listing for the life of the handle

Not on `main`. Lives on `claude/aloelite-ci-benchmarks-9ts6he`.

**Both daemons rebuilt the whole directory listing on every kernel
`readdir` continuation call.** The kernel reads a directory in
buffer-sized instalments, calling `readdir` again and again with a
rising offset, so a scan cost one full listing PER CALL -- and since a
listing is already O(N^2) (`directory_listing` resolves NODE-5
visibility with a correlated subquery per row), a scan through a mount
was close to cubic.

`AloeFuse.opendir` and `Inner::opendir` now snapshot the visible
listing, `readdir` serves every instalment from it, and `releasedir`
drops it. The Python handle table is a separate `_dirs` dict and the
Rust one a separate `dirs` map, so nothing that walks FILE handles
(flush, release, the getattr size overlay) has to know directories
exist.

Measured, one `readdir` of a 5,000-entry directory:
...
Fitted growth falls from n^2.94 to n^2.0 (Python) and n^2.74 to n^2.1
(Rust). At 10,000 entries neither daemon could be measured before --
the benchmark projected 2,524 s and 592 s and skipped both -- and both
now finish in about the time the library itself takes (52 s and 47 s
against 51 s). The mount's own contribution to readdir cost is gone;
what remains is the O(N^2) listing underneath, which is a schema
question (an index on `node (name)`, or D-5's `edge.name`) and not the
daemons'.

The snapshot also fixes the offset cookies, which were never stable: a
readdir offset has to mean the same entry on the next call, and against
a freshly built listing an entry created or removed mid-scan shifted
every cookie after it, so the kernel could skip or repeat entries. A
snapshot cannot shift. POSIX leaves concurrent modification during a
scan unspecified, which is what lets both daemons take the stable
reading; an entry unlinked after the snapshot is omitted rather than
failing the whole scan.

### Benchmark suite and CI benchmark job
...
- Throughput, small-file ops, random `pread` p50/p99, and append commit
  ...
  durability barrier for every backend (an ext4 buffered write and a WAL
  commit are not the same promise).
- Space: on-disk over logical for unique / template-cloned / sparse
  corpora; dedup within a volume and ACROSS volumes, which is the row
  that confirms per-volume key separation (plain 1.0, encrypted 0.0);
  bytes per version for a small edit; prune's freed pages reported
  separately from VACUUM's file shrink.
- Scale curves rather than averages: ingest rate against chunk-table
  depth, lookup and readdir against directory size, and peak RSS for a
  long stream (measured in a child, since `ru_maxrss` never falls).
- Maintenance: unlock and `change_pin` against volume size, verify
...
`doc/BENCHMARKS.md` records the method, the findings, and what the
harness deliberately does not measure. Nothing asserts: the job fails
only when a suite crashes, never on a slow number.

**Finding worth acting on:** directory size is a scaling wall, and one
missing capability causes both halves of it. `name` lives on `node` and
`from_id` lives on `edge`, so no index can answer "the child of
container C named X" as a seek. `resolve_segment` (and the same shape
inside `resolve_path`'s CTE) therefore seeks `edge_from_active` on
`from_id` alone and filters the container's whole child set by name,
making a single `stat` O(entries in that directory); and
`directory_listing` runs that as a correlated scalar subquery per row,
making `readdir` O(N^2). Measured against ext4 on the same disk, which
is flat on lookup (htree) and linear on readdir: at 10,000 entries
aloelite needs 6.2 ms to stat one file where ext4 needs 0.005 ms, and
49 s to list the directory where ext4 needs 2.9 ms -- 1,100x and
~17,000x. The exponent is the problem, not the constant. Candidate
fixes
are an index on `node (name)` (additive) or denormalising the name onto
the edge, which D-5 already plans -- a schema question that belongs
with the era-2 work, not with a benchmark.

Through a mount it was worse again, for a second and independent
reason, and in BOTH daemons -- the listing was rebuilt on every kernel
continuation call. That half needed no schema change and is fixed; see
the entry above.
