...
- **Release artifacts carry no version, and the checksum manifest is
  ...
  `aloelite_linux_x86_64_gnu`, a bare binary. Each platform now ships
  its binaries separately -- the CLI, and `aloelite_fuse_*` where the
  daemon builds -- plus one `aloelite_<platform>_complete` bundle
  carrying everything built for it. The wasm leaves are
  `aloelite_wasi.wasm` and `aloelite_web.tar.gz`. The wheel and sdist
  keep their versioned names, which PyPI mandates.
