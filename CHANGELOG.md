...
- **New crate: `aloelite-extism`, Aloelite as an Extism plug-in.**
  Five exports -- `fs_open`, `fs_open_memory`, `fs_call`, `fs_close`,
  `fs_operations` -- over MessagePack, dispatching the same
  `aloelite-api` table the browser does. A host in Go, Python, Ruby,
  PHP, Java, C# or anything else with an Extism SDK gets the Mount API
  without a binding written for its language, and gets it inside a
  sandbox that reaches exactly the paths the manifest names.
