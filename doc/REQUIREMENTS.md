...
- **ACC-11** : A lock excludes another mount from changing the locked node's content, its metadata map, its placement, or its existence. Content writes, `set_metadata`, `move`, `rename`, `link` and `remove` are all refused at the locked node. `rename` and `link` were added in the era-2 reconciliation: both change placement, which this requirement already named, and `rename` mattered most because FUSE routes a same-directory `mv` to it and a cross-directory `mv` to `move`, so one gesture honoured the lock and the other walked past it. Destruction is additionally checked *transitively*: a recursive remove is refused if any member of the subtree is locked, because it would archive that member. Relocation is **not** transitive — moving an ancestor of a locked node is permitted, since it changes the node's path but neither its content nor its existence, and checking ancestors would make one open file freeze every directory above it.
