...
- **Rust** (`rust/`) — eight crates on four targets. `aloelite-core` is the engine and compiles unchanged for native, WASI and the browser; `aloelite-api` is the Mount API dispatched by name, which every frontend shares; `aloelite-cli` and `aloelite-fuse` are the native frontends, parsed from the same verb contract as the Python ones; `aloelite-wasm` is the browser surface, and `aloelite-extism` is an Extism plug-in that puts the engine in a sandbox any language can call. The conformance suite is what holds the two implementations to one behaviour, and `doc/BENCHMARKS.md` measures them against each other.
